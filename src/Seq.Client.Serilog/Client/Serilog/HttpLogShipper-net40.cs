...
namespace Seq.Client.Serilog
{
    class HttpLogShipper : IDisposable
    {
            ...
            try
            {
                ...
                do
                {
                    ...
                    using (var bookmark = File.Open(_bookmarkFilename, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.Read))
                    {
                        ...
                        string currentFile;
                        var bookmarkWrapper = new NonClosingStreamWrapper(bookmark);

                        TryReadBookmark(bookmarkWrapper, out nextLineBeginsAtOffset, out currentFile);
                        ...
                        if (currentFile != null)
                        {
                            ...
                            {
                                var currentWrapper = new NonClosingStreamWrapper(current);
                                currentWrapper.Position = nextLineBeginsAtOffset;

                                string nextLine;
                                while (count < _batchPostingLimit &&
                                    TryReadLine(currentWrapper, ref nextLineBeginsAtOffset, out nextLine))
                                {
                                    ++count;
                                ...
                                payload.Write("]}");
                                currentWrapper.BaseStream.Close();
                            }
                            ...
                            if (count > 0)
                            {
                                ...
                                {
                                    WriteBookmark(bookmarkWrapper, nextLineBeginsAtOffset, currentFile);
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                {
                                    WriteBookmark(bookmarkWrapper, 0, fileSet[1]);
                                }
                        ...
                        bookmarkWrapper.BaseStream.Close();
                    }
                }
                while (count == _batchPostingLimit);
            }
        ...
        static void WriteBookmark(NonClosingStreamWrapper bookmark, long nextLineBeginsAtOffset, string currentFile)
        {
        ...
        static bool TryReadLine(NonClosingStreamWrapper current, ref long nextStart, out string nextLine)
        {
        ...
        static void TryReadBookmark(NonClosingStreamWrapper bookmark, out long nextLineBeginsAtOffset, out string currentFile)
        {
            nextLineBeginsAtOffset = 0;
            currentFile = null;
